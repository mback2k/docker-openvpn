...
echo "exec /usr/sbin/openvpn --user ${OPENVPN_USER} --daemon ovpn-${OPENVPN_NETNAME} --status /run/openvpn/${OPENVPN_NETNAME}.status 10 --cd /etc/openvpn --config /etc/openvpn/${OPENVPN_NETNAME}.conf" >> /usr/local/sbin/openvpn
